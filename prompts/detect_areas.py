...
Rules:
1. Allowed types: header | footer | page_number | illustration | main_text | illustration_caption | footnote | decoration | chapter_title | marginalia | table
...
4. Provide "text" (verbatim OCR transcription or plain text representation) for types: main_text, footnote, illustration_caption, chapter_title, header, footer, page_number, marginalia, table. For "table", provide a clean plain-text/tab-separated representation in "text", and provide a well-structured, clean HTML table in "table_html". For all other types, set "table_html" to null. Set both to null for illustration and decoration.
...
11. There may be multiple areas of types: main_text, illustration, illustration_caption, footnote, table.
...
14. For tables, create a single area that will encompass the entire table. Set its type to "table" (never classify it as "illustration" or "main_text"). For the "table_html" field, convert the table into valid, semantic, clean HTML <table> markup (using <table>, <thead>, <tbody>, <tr>, <th>, <td>, and standard rowspan/colspan attributes for merged cells; do not add CSS styles, class attributes, or markdown formatting inside the JSON string).
...
16. Bold and italic text styles: For main_text and table cell areas, wrap bold text in <strong> and italic text in <i> HTML tags. \
Only <strong> and <i> are permitted inside the text content or table_html cells — no other HTML styling/formatting tags may appear in text content.
"""
